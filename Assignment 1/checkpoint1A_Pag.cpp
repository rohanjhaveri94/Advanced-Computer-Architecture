...
bool BTB_prediction(ADDRINT ins_ptr)
{
    ...
    int predict;
    predict = BTB_Table[BTB[index].History & 0x3FF];
...
VOID BTB_update(ADDRINT ins_ptr, bool taken)
{
    ...
    if(taken){
	BTB_Table[BTB[index].History & 0x3FF]++;
	 if ( BTB_Table[BTB[index].History & 0x3FF]>3){
               BTB_Table[BTB[index].History & 0x3FF] = 3;}}
     else{
	if(BTB_Table[BTB[index].History & 0x3FF] != 0){
        BTB_Table[BTB[index].History & 0x3FF]--;}}
