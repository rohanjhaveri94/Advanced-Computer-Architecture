...
void FilterWrapper(unsigned char* pImageIn, int Width, int Height, float* d_sobel);
...
unsigned char *d_Out;
float *d_sobel; 
float sobel_matrix[25]={1,2,0,-2,-1,4,8,0,-8,-4,6,12,0,-12,-6,4,8,0,-8,-4,1,2,0,-2,-1};
...
const int EDGE_VALUE_THRESHOLD = 1170;
const int HIGH_BOOST_FACTOR = 10;

#include "filter_kernel_5x5.cu"
...
int main(int argc, char** argv)
{
 ...
 d_sobel = (float*)malloc(25*sizeof(float));
    if (d_sobel == 0) Cleanup();
 ...
 cudaMemcpy(d_sobel, sobel_matrix, 25*sizeof(float), cudaMemcpyHostToDevice);
    cutilCheckError(cutStopTimer(sobel_mtimer));

 printf("1st Transfer %f (ms)\n", cutGetTimerValue(sobel_mtimer));  
  
    // Call Kernels
    FilterWrapper(data, dib.width, dib.height, d_sobel);
...
void FilterWrapper(unsigned char* pImageIn, int Width, int Height, float* d_sobel)
{
   ...
   switch(FilterMode) {
     ...
     SobelFilter<<< dimGrid, dimBlock >>>(d_In, d_Out, Width, Height, d_sobel);
